...
<head>
	...
	<?php if (DEMO || USECOMPILEDASSET) : ?>
		...
		<link href="../assets/midpos/cssmin/pos<?php echo isRTL() ? '-rtl' : null;?>.css" type="text/css" rel="stylesheet">
	...
	<?php else : ?>
		...
		<!-- Filemanager CSS -->
	    <link href="../assets/midpos/css/filemanager/dialogs.css" type="text/css" rel="stylesheet">
	    <link href="../assets/midpos/css/filemanager/main.css" type="text/css" rel="stylesheet">

	    <!-- Theme CSS -->
	    <link href="../assets/midpos/css/theme.css" type="text/css" rel="stylesheet">

	    <!-- Skin Black CSS -->
	    <link href="../assets/midpos/css/skins/skin-black.css" type="text/css" rel="stylesheet">

	    <!-- Skin Blue CSS -->
	    <link href="../assets/midpos/css/skins/skin-blue.css" type="text/css" rel="stylesheet">

	    <!-- Skin Green CSS -->
	    <link href="../assets/midpos/css/skins/skin-green.css" type="text/css" rel="stylesheet">

	    <!-- Skin Red CSS -->
	    <link href="../assets/midpos/css/skins/skin-red.css" type="text/css" rel="stylesheet">

	    <!-- Skin Yellow CSS -->
	    <link href="../assets/midpos/css/skins/skin-yellow.css" type="text/css" rel="stylesheet">

	    <!-- Main CSS -->
	    <link href="../assets/midpos/css/main.css" type="text/css" rel="stylesheet">

		<!-- Skeleton CSS -->
		<link href="../assets/midpos/css/pos/skeleton.css" rel="stylesheet" type="text/css">

		<!-- Main CSS -->
		<link href="../assets/midpos/css/pos/pos.css" rel="stylesheet" type="text/css">

		<!-- Responsive CSS -->
		<link href="../assets/midpos/css/pos/responsive.css" rel="stylesheet" type="text/css">
	...
	<style type="text/css">
		body::after { 
			content: ""; background: url(../assets/midpos/img/pos/patterns/<?php echo $user->getPreference('pos_pattern') ? $user->getPreference('pos_pattern') : 'armysuit.jpg'; ?>) repeat repeat;opacity: 0.4;filter: alpha(opacity=40);top: 0;left: 0;bottom: 0;right: 0;position: absolute;z-index: -1;
		}
	...
	<?php if (LOCALIZATION):?>
		<script type="text/javascript">
			var localCurrency = "<?php echo LOCAL_CURRENCY;?>";
			var localization = "<?php echo LOCALIZATION;?>";
			var curValue = [];
			<?php foreach (get_currencies() as $curr):?>
			curValue['<?php echo $curr['code'];?>'] = '<?php echo $curr['value'];?>';
			<?php endforeach;?>
		</script>
	<?php endif;?>
</head>
...
<?php include('../_inc/template/pos_skeleton.php'); ?>
	...
	<div class="pos-content-wrapper">
		...
		<?php include('../_inc/template/partials/top1.php'); ?>
						...
						<div class="contents">
							...
							<div id="item-list">
								...
								<div ng-show="showLoader" class="ajax-loader">
									<img src="../assets/midpos/img/loading2.gif">
								</div>
							...
							<div id="total-amount">
								...
								<?php if (LOCALIZATION):?>
								<div id="local-currency-wrap" style="position: absolute!important;display: block;top: 50%;right: 137px;width: 30px;height: 30px;margin-top: -15px;cursor: pointer;">
									<select class="form-control" id="local-currency" name="local-currency">
										<?php foreach(get_currencies() as $curr) : ?>
										  	<option value="<?php echo $curr['code'];?>"<?php echo $curr['code'] == LOCAL_CURRENCY ? ' selected' : null;?>>
										  		<?php echo $curr['title']; ?>
										  	</option>
										<?php endforeach; ?>
									</select>
								...
								<?php endif;?>
								<div id="created_at">
									<input ng-init="createdAt='<?php echo date('Y-m-d');?>'" type="date" name="created_at" value="<?php echo date('Y-m-d');?>" ng-model="createdAt">
								</div>
						...
						<div class="invoice-area">
							<div class="well well-sm">
										...
										<div class="previous-due-inner">
											<h4>
												...
												<a ng-show="dueAmount" href="customer_profile.php?customer_id={{ customerId }}&type=all_due" target="_blink">
													<span id="dueAmount">
														{{ dueAmount | curConvert:conValue:currencyCode | formatDecimal:2 }}
													</span>
												</a>
											...
											<tbody>
												<tr ng-repeat="items in itemArray" class="invoice-item">
													<td class="product-quantity" id="invoice-item-{{ items.id }}">
														...
														<button class="btn btn-xs btn-up" ng-click="addItemToInvoice(items.id, items)" title="Increase">
													...
													<td class="product-name">
														<span>{{ items.name }} 
															<small ng-show="items.hasVariant">({{ items.variantSlug }})</small> &nbsp;<i ng-show="items.hasVariant" ng-click="SellOptionSelectorModal(items)" class="fa fa-fw fa-th-large pull-right pointer" style="margin-top:4px;" title="<?php echo trans('Select Option');?>"></i>&nbsp;
															<i ng-click="addItemNote(items);" class="fa fa-comments-o pointer pull-right poduct-eye" style="margin-top:3px;"></i>
														</span>
													</td>
													<td class="product-price">
														<?php if (get_preference('change_item_price_while_billing') == 1) : ?>
															<input type="text" class="text-center item_price" id="item_price_{{ items.id }}" name="item_price_{{ items.id }}" value="{{ items.price | curConvert:conValue:currencyCode | formatDecimal:2 }}" data-itemid="{{ items.id }}" onClick="this.select();" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" style="max-width:80px;padding:5px;border-radius: 20px;border:2px solid #ddd;">
														<?php else : ?>
															{{ items.price | curConvert:conValue:currencyCode | formatDecimal:2 }}
														<?php endif; ?>
													</td>
													<td class="product-subtotal">
														{{ items.subTotal | curConvert:conValue:currencyCode | formatDecimal:2 }}
													</td>
								...
								<div id="pay-button" class="text-center">
									<div ng-hide="isEditMode" class="btn-group btn-group-justified">
										<div class="btn-group">
											<button ng-show="!ref_no" ng-click="orderNow()" onClick="return false;" class="btn btn-info" data-loading-text="Processing..." title="Order Now">
												<span class="fa fa-fw fa-money"></span> 
												<?php echo trans('button_order'); ?>
											</button>
										</div>
										<div class="btn-group">
											<button ng-show="!ref_no" ng-click="HoldingOrderModal()" on-click="return false;" class="btn btn-primary" data-loading-text="Processing..." title="Order Holdinbg">
									...
									<div ng-hide="isEditMode" class="btn-group btn-group-justified">
										<div class="btn-group">
											<button ng-click="payNow()" onClick="return false;" class="btn btn-success" data-loading-text="Processing..." title="Payment Now">
												<span class="fa fa-fw fa-money"></span> 
												<?php echo trans('button_pay'); ?>
											</button>
										</div>
										<div class="btn-group">
											<button ng-click="resetPos()" on-click="return false;" class="btn btn-danger" data-loading-text="Processing..." title="Reset POS">
												<span class="fa fa-fw fa-close"></span> 
												<?php echo trans('button_cancel'); ?>
											</button>
										</div>
									</div>
									<div ng-show="isEditMode" class="btn-group btn-group-justified">
										<div class="btn-group">
											<button ng-click="updateNow()" onClick="return false;" class="btn btn-success" data-loading-text="Updating..." title="Update Now">
	...
	<?php if (DEMO || USECOMPILEDASSET) : ?>
		...
		<script src="../assets/midpos/jsmin/pos.js" type="text/javascript"></script>
	...
	<?php else : ?>
		...
		<!-- POS Main JS -->
		<script src="../assets/midpos/js/pos/pos.js" type="text/javascript"></script>

<?php endif; ?>

<script src="../assets/midpos/angular/modals/SellOptionSelectorModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/AddInvoiceNoteModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/AddItemNoteModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/AddCustomerMobileNumberModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/OrderNowModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/HoldingOrderModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/modals/HoldingOrderDetailsModal.js" type="text/javascript"></script>
<script src="../assets/midpos/angular/controllers/PosController.js" type="text/javascript"></script>
<noscript>
    <div class="global-site-notice noscript">
        <div class="notice-inner">
            <p><strong>JavaScript seems to be disabled in your browser.</strong><br>You must have JavaScript enabled in
                your browser to utilize the functionality of #Mid Pos.</p>
        </div>
    </div>
</noscript>
</body>
