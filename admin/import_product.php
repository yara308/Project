			...
			switch ($Name) {
				case 'Product':
					foreach ($Reader as $Row)
					{
						...
						$pro_data['gender_for'] = isset($Row[13]) ? strtolower($Row[13]) : NULL;
						...
						$statement = db()->prepare("SELECT * FROM `products` WHERE `p_name` = ? OR `p_code` = ?");
      					...
      					if (!$product) {
							...
							$statement = db()->prepare("INSERT INTO `products` (`p_type`, `p_code`, `hsn_code`, `barcode_symbology`, `p_name`, `category_id`, `gender_for`, `unit_id`, `p_image`, `description`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			        		$statement = $statement->execute(array($pro_data['product_type'], $p_code, $pro_data['hsn_code'], $pro_data['barcode_symbology'], $pro_data['product_name'], $pro_data['category_id'], $pro_data['gender_for'], $pro_data['unit_id'], $pro_data['thumbnail'], $pro_data['description']));
							...
							$statement = db()->prepare("UPDATE `products` SET  `p_type` = ?, `p_code` = ?, `hsn_code` = ?, `barcode_symbology` = ?, `p_name` = ?, `category_id` = ?, `gender_for` = ?, `unit_id` = ?, `p_image` = ?, `description` = ? WHERE `p_id` = ?");
			      			$statement->execute(array($pro_data['product_type'], $p_code, $pro_data['hsn_code'], $pro_data['barcode_symbology'], $pro_data['product_name'], $pro_data['category_id'], $pro_data['gender_for'], $pro_data['unit_id'], $pro_data['thumbnail'], $pro_data['description'], $product_id));
