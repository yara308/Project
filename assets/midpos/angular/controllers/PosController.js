...
function (
    ...
    if (window.localization) {
        $('#local-currency').on('select2:select', function (e) {
            var code = e.params.data.element.value;
            $scope.conValue = curValue[code];
    ...
    $scope.addCustomer = function (customer) {
        if ($scope._isInt(customer)) {
            ...
            $scope.customerMobileNumber = customer.ddcustomer_mobile;
    ...
    $scope._calcDisAmount = function () {
        ...
        $scope.totalDiscountAmount = $scope.discountType  == 'percentage' ? $scope._percentage($scope.totalAmount, $scope.discountAmount) : $scope.discountAmount;
    };
    $scope._calcTaxAmount = function () {
        if ($scope.taxInput < 0 || $scope.taxInput > 100) {
            ...
            $scope.taxAmount = (parseFloat($scope.taxInput) / 100) * (parseFloat($scope.totalAmount)-parseFloat($scope.itemTaxAmount));
        }
    };
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        $scope.payable = ($scope.payable + shippingPercentage + $scope.othersCharge + $scope.dueAmount) - $scope.totalDiscountAmount;
    ...
    $scope.addItemToInvoice = function (id, qty, index) {
        ...
        var proQty;
        var variantSlug;
        if (qty.variantSlug && qty.variantSlug != undefined) {
            variantSlug = qty.variantSlug;
        }
        qty = parseFloat(qty);
        if (!qty) {
            qty = 1;
        ...
        var $queryString = "p_id=" + id + "&action_type=PRODUCTITEM&variant_slug="+variantSlug;
                ...
                if (find) {
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            if (item.variantSlug && item.variantSlug != undefined) {
                                window._.map(response.data.variants, function (variant) {
                                    if (!$scope.isEditMode) {
                                        if (variant.variant_slug == item.variantSlug && parseFloat(variant.quantity) <= parseFloat(item.quantity)) {
                                            if (window.store.sound_effect == 1) {
                                                window.storeApp.playSound("error.mp3");
                                            }
                            ...
                            if (error == false) {
                                ...
                                if (!$scope.isEditMode && (qty > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) && response.data.p_type != 'service') {
                                    if (window.store.sound_effect == 1) {
                                        window.storeApp.playSound("error.mp3");
                                    }
                                    window.toastr.error("This product is out of stock!", "Warning!");
                                ...
                                item.subTotal = (item.subTotal + (parseFloat(response.data.sell_price) * qty)) + taxamount + (parseFloat(response.data.sell_price_addition) * qty);
                                $scope.totalQuantity = $scope.totalQuantity + qty;
                                $scope.totalAmount = $scope.totalAmount + (parseFloat(response.data.sell_price) * qty) + taxamount + (parseFloat(response.data.sell_price_addition) * qty);
                            }
                        }
                    });
                } else {
                    if (!$scope.isEditMode && (qty > response.data.quantity_in_stock) && response.data.p_type != 'service') {
                        if (window.store.sound_effect == 1) {
                            window.storeApp.playSound("error.mp3");
                        }
                        window.toastr.error("This product is out of stock!", "Warning!");
                    ...
                    item.hasVariant = parseInt(response.data.has_variant);
                    item.variantID = response.data.variant_id;
                    item.variantSlug = response.data.variant_slug;
                    item.variantName = response.data.variant_name;
                    ...
                    item.price = parseFloat(response.data.sell_price) + taxamount + parseFloat(response.data.sell_price_addition);
                    item.quantity = qty;
                    item.subTotal = (parseFloat(response.data.sell_price) * qty) + taxamount + (parseFloat(response.data.sell_price_addition) * qty);
                    $scope.totalQuantity = $scope.totalQuantity + qty;
                    $scope.totalAmount = $scope.totalAmount + (parseFloat(response.data.sell_price) * qty) + item.taxamount + (parseFloat(response.data.sell_price_addition) * qty);
    ...
    $scope.SellOptionSelectorModal = function(item) {
        $scope.item = item;
        SellOptionSelectorModal($scope);
    }

    $scope.addItemNote = function(item) {
        $scope.item = item;
        AddItemNoteModal($scope);
    }
    ...
    if (window.getParameterByName("invoice_id")) {
        ...
        $http({
            method: "GET",
            url: window.baseUrl + "/_inc/pos.php?action_type=SELLEDITINFO&invoice_id="+$scope.invoiceId,
            dataType: "JSON"
        })
        .then(function(res) {      
            window.angular.forEach(res.data.items, function(item, key) {
                $scope.addItemToInvoice(item.item_id, item.item_quantity); 
            });
            $scope.$applyAsync(function() {
                $scope.addCustomer(res.data.info.customer_id);
                $scope.totalItem        = window.formatDecimal(res.data.info.total_items,2);
                $scope.totalQuantity    = 0;
                $scope.discountAmount   = window.formatDecimal(res.data.info.discount_amount,2);
                $scope.discountInput    = window.formatDecimal(res.data.info.discount_amount,2);
                $scope.taxAmount        = window.formatDecimal((parseFloat(res.data.info.order_tax)/(parseFloat(res.data.info.subtotal)))*100,2);
                $scope.taxInput         = window.formatDecimal((parseFloat(res.data.info.order_tax)/(parseFloat(res.data.info.subtotal)))*100,2);
                $scope.itemTaxAmount    = window.formatDecimal(res.data.info.item_tax,2);
                $scope.shippingAmount   = window.formatDecimal(res.data.info.shipping_amount,2);
                $scope.shippingInput    = window.formatDecimal(res.data.info.shipping_amount,2);
                $scope.othersCharge     = window.formatDecimal(res.data.info.others_charge,2);
                $scope.othersChargeInput= window.formatDecimal(res.data.info.others_charge,2);
                $scope.totalAmount      = 0;
                $scope.payable          = 0;
                $scope.totalPayable     = 0;
                $scope.paidAmount       = window.formatDecimal(res.data.info.paid_amount,2);
                $scope.pmethodId        = res.data.info.pmethod_id;
                $("#invoice-note").data("note",res.data.info.invoice_note);
                setTimeout(function() {
                    window.angular.forEach(res.data.items, function(item, key) {
                        var find = window._.find($scope.itemArray, function (theItem) {
                            return item.item_id == theItem.id;
                        });
                        if (find) {
                            window._.map($scope.itemArray, function (theItem) {
                                if (item.item_id == theItem.id) {
                                    theItem.notes = item.notes;
                                }
                            });
                        }
                    });
    ...
    $scope.loadQuotationOrder = function() {
        if (window.getParameterByName("qref")) {
            ...
            then(function(response) {
                ...
                if (quotation.is_order && !quotation.is_approve) {
                    window.toastr.error('Unapproved order', "warning!");
                    if (window.store.sound_effect == 1) {
                        window.storeApp.playSound("error.mp3");
                    }
    ...
    $scope.resetPos = function (force) {
        ...
        if (window.getParameterByName("invoice_id") || force == 1 && (window.getParameterByName("customer_id") || window.getParameterByName("holding_id") || window.getParameterByName("qref"))) {
            window.location = "pos.php";
        } else {
            $scope.customerArray    = [];
            $scope.itemArray        = [];
    ...
    $(document).on('click', function(e) {
        if ($scope.error == false) {
            window._.map($scope.itemArray, function (item) {
                ...
                var $queryString = "p_id=" + item.id + "&action_type=PRODUCTITEM&is_edit_mode="+$scope.isEditMode;
                $http({
                    url: API_URL + "/_inc/pos.php?" + $queryString,
                    method: "GET",
                    cache: false,
                ...
                then(function(response) {
                    if (response.data.p_id) {
                        if (!$scope.isEditMode && (itemquantity > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) &&  response.data.p_type != 'service') {
                            if ($scope.error == false) {
                                $scope.error = true;
    ...
    $.contextMenu.types.label = function(item, opt, root) {
        ...
        .on("click", "button", function() {
            ...
            if (!itemQuantity || parseFloat(itemQuantity) <= 0) {
                ...
                window.toastr.error(trans('error_quantity_must_be_greater_than_zero'), "Warning!");
                return false;
            }
    ...
    $("#item-list").contextMenu({
        ...
        items: {
            "add": {name: trans('text_add_a_item'), icon: "fa-plus"},
            "sep1": "---------",
            "add_specific_amount": {name: trans('text_add_specific_quantity'), icon: "fa-th", disabled: true},
            "quantity": {
                type: "label", 
                customName: trans('text_quantity'), callback: function() { 
                    $scope.productItemId = $(this).find(".item-info").data("id");
                    return false; 
                },
            },
            "view": {name: trans('text_view'), icon: "fa-eye"},
            "sep2": "---------",
            "edit": {name: trans('text_edit'), icon: "fa-pencil"}
        }
    });
