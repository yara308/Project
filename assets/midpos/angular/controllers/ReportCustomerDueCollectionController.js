...
function (
    ...
    $("#invoice-invoice-list").dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/report_customer_due_collection.php?user_id="+$userId+"&store_id=" + $store_id + "&from="+$from+"&to="+$to,
        "fixedHeader": true,
        "order": [[ 0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
    ...
    $(document).delegate("#user_id", "select2:select", function (e) {
        ...
        window.location = window.baseUrl+"/admin/report_customer_due_collection.php?user_id="+data.element.value;
    });
    if (window.getParameterByName('user_id')) {
        $("#user_id").val(window.getParameterByName('user_id')).trigger("change");
    }
