...
function (
    ...
    dt.dataTable({
        ...
        "dom": "lfBrtip",
        "serverSide": true,
        "ajax": API_URL + "/_inc/transfer.php?from=" + $from + "&to=" + $to+ "&type=" + $type + "&store_id=" + $store_id,
		"fixedHeader": true,
        "order": [[0, "desc"]],
        "aLengthMenu": [
            [10, 25, 50, 100, 200, -1],
        ...
        "buttons": [
            {
                ...
                customize: function ( win ) {
                    $(win.document.body)
                        .css( 'font-size', '10pt' )
                        .append(
                            '<div><b><i>Developed & Maintained by: Onzstore.com</i></b></div>'
                        )
                        .prepend(
                            '<div class="dt-print-heading"><img class="logo" src="'+baseUrl+'/assets/midpos/img/logo-favicons/1_logo.png"/><h2 class="title">'+window.store.name+'</h2><p><b><i>From</i> '+window.from +' <i>To</i> '+window.to+'</b><br>Printed on: '+window.formatDate(new Date())+'</p></div>'
    ...
    $scope.addItemToTransferList = function (id, item_variant_slug, qty, index) {
        if (!qty) { qty = 1; }
        $http({
            url: API_URL + "/_inc/ajax.php?type=STOCKITEM&id="+id+"&item_variant_slug="+item_variant_slug+"&quantity="+qty,
            method: "GET",
            cache: false,
            processData: false,
            contentType: false,
        ...
        then(function(response) {
            if (id) {
                var find = window._.find($scope.tItemArray, function (item) { 
                    return item.item_id == response.data.product.id && item.item_variant_slug == response.data.product.item_variant_slug;
                });
                if (find) {
                    window._.map($scope.productsArray, function (sitem) {
                        if (sitem.item_id == response.data.product.id && sitem.item_id == response.data.product.id && sitem.item_variant_slug == response.data.product.item_variant_slug) {
                            if (sitem.quantity <= 0) {
                                window.toastr.warning('The item is out of stock', "Warning!");
                                stopProcess = true;
                            } else {
                    ...
                    if (stopProcess == false) {
                        window._.map($scope.tItemArray, function (item) {
                            if (item.item_id == response.data.product.id && item.item_variant_slug == response.data.product.item_variant_slug) {
                                item.quantity = item.quantity + 1;
                                $scope.totalItem = window._.size($scope.tItemArray);
                            }
                        });
                    }
                } else {
                    response.data.product.quantity = 1;
                    $scope.tItemArray.push(response.data.product);
                    $scope.totalItem = window._.size($scope.tItemArray);
                    window._.map($scope.productsArray, function (sitem) {
                        if (sitem.item_id == response.data.product.id && sitem.item_variant_slug == response.data.product.item_variant_slug) {
                            // sitem.quantity = sitem.quantity-1;
                        }
                    });
                }  
    ...
    $scope.removeItemFromList = function (index, id) {
        window._.map($scope.tItemArray, function (item, key) {
            // var quantity = parseFloat($("#quantity-"+item.item_id).val());
            var quantity = item.quantity;
            if (item.item_id == id) {
                $scope.totalItem = $scope.totalItem - 1;
            }
            window._.map($scope.productsArray, function (sitem) {
                if (sitem.item_id == item.item_id) {
                    sitem.quantity = sitem.quantity+quantity;
                }
            });
        });
        $scope.tItemArray.splice(index, 1);
        $scope.totalItem = window._.size($scope.tItemArray);
        stopProcess = false;
    };
    ...
    $scope.stockCheck = function() {
        window._.map($scope.tItemArray, function (item) {
            ...
            if (isNaN(quantity)) {
                quantity = 1;
                $("#quantity-"+item.item_id).val(quantity);
            }
            window._.map($scope.productsArray, function (sitem) {
                // if (sitem.item_id == item.item_id) {
                if (sitem.item_id == item.item_id && sitem.item_variant_slug == item.item_variant_slug) {
                    var stockQuantity = sitem.item_quantity - sitem.total_sell;
                    if (stockQuantity <= quantity) {
                        window.toastr.warning('Out of Stock', "Warning!");
                        stopProcess = true;
                        sitem.quantity = 0;
                        $("#quantity-"+sitem.item_id).val(stockQuantity);
                        return false;
                    } else {
