window.angularApp.factory("SellOptionSelectorModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "$rootScope", function(API_URL, window, $, $http, $uibModal, $sce, $scope) {
    return function($scope) {
        ...
        var uibModalInstance = $uibModal.open({
            ...
            template: "<div class=\"modal-header\">" +
                           "<button ng-click=\"closeSellOptionSelectorModal();\" type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-label=\"Close\"><span aria-hidden=\"true\">&times;</span></button>" +
                        ...
                        "<div class=\"modal-footer\">" +
                            "<button ng-click=\"closeSellOptionSelectorModal();\" type=\"button\" class=\"btn btn-info\"><span class=\"fa fa-fw fa-check\"></span> OK &nbsp;</button>&nbsp;&nbsp;" +
                        "</div>",
            controller: function ($scope, $uibModalInstance) {
                $http({
                  url: window.baseUrl + "/_inc/ajax.php?id=" + item.id + "&name=" + item.name + "&type=SELLPRODUCTOPTIONS",
                  method: "GET"
                })
                .then(function(response, status, headers, config) {
                    ...
                    .then(function() {
                        $scope.closeSellOptionSelectorModal();
                    });
                });
                        ...
                        window._.map($scope.itemArray, function (item) {
                            if (item.id == id) {
                                if (item.quantity > 0) {
                                    ...
                                    item.price = parseFloat(price) + parseFloat(priceAddition);
                                } else {
                                    if (window.store.sound_effect == 1) {
                ...
                $scope.closeSellOptionSelectorModal = function () {
