...
function (
    ...
    $scope.loadStockReport = function(supID) {
        ...
        stockReportDT.dataTable({
            ...
            "dom": "lfBrtip",
            "serverSide": true,
            "ajax": API_URL + "/_inc/report_stock.php?sup_id=" + supID + "&from=" + $from + "&to=" + $to + "&store_id=" + $store_id,
            "fixedHeader": true,
            "aaSorting": [],
            "aLengthMenu": [
                [10, 25, 50, 100, 200, -1],
            ...
            "drawCallback": function ( settings ) {
                ...
                api.column(1, {page:'current'} ).data().each( function ( group, i ) {
                    ...
                    if ( last !== group ) {
                        $(rows).eq( i ).before(
                            '<tr class="group warning" data-child="'+generateHashCode(group)+'"><td class="bg-green" colspan="2" data-title="'+trans('label_supplier_name')+'"><i class="fa fa-fw fa-angle-right"></i> '+group+'</td><td class="bg-green '+availabilityRow+'  text-right" data-title="'+trans('label_available')+'"></td><td class="bg-green '+totalSellPriceRow+'  text-right" data-title="T'+trans('label_sell_price')+'"></td><td class="bg-green '+totalpurchasePriceRow+'  text-right" data-title="'+trans('label_purchase_price')+'"></td></tr>'
                        );
                        last = group;
                    } 
