...
function (
    ...
    if (dt.length > 0) {
        ...
        dt.dataTable({
            ...
            "dom": "lfBrtip",
            "serverSide": true,
            "ajax": API_URL + "/_inc/order.php?from="+$from+"&to="+$to+"&type="+$type + "&store_id=" + $store_id,
            "order": [[ 0, "desc"]],
            "aLengthMenu": [
                [10, 25, 50, 100, 200, -1],
    ...
    $scope.addProduct = function(data) {
        ...
        html += "<input name=\"products["+data.itemId+"][variant_name]\" type=\"hidden\" class=\"variant-name\" value=\""+data.variantName+"\">";
        html += "<input name=\"products["+data.itemId+"][variant_slug]\" type=\"hidden\" class=\"variant-slug\" value=\""+data.variantSlug+"\">";
        html += "<input name=\"products["+data.itemId+"][sup_id]\" type=\"hidden\" class=\"sup_id\" value=\""+data.supId+"\">";
        ...
        .then(function (data) {
            ...
            window.angular.forEach(order.items, function(item, key) {
                var data = {
                    ...
                    variantSellPriceAddition: item.sell_price_addition,
