...
window.angularApp.factory("ProductEditModal", ["API_URL", "window", "jQuery", "$http", "$uibModal", "$sce", "POSFilemanagerModal", "$rootScope", function (API_URL, window, $, $http, $uibModal, $sce, POSFilemanagerModal, $scope) {
    return function(product) {
        var productId;
        $scope.promotionalPriceArray = [];
        $scope.variantArray = [];
        $scope.promotionalPriceSerial = 0;
        $scope.variantSerial = 0;
        $scope.imgArray = [];
        $scope.imgSerial = 0;
        var uibModalInstance = $uibModal.open({
            // animation: true,
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                .then(function(response, status, headers, config) {
                    ...
                    setTimeout(function() {
                        ...
                        var hasPromotionalPrice =  $("#has_promotional_price").is(":checked");
                        $scope.updatePromotionalPrice = function() {
                            $scope.$apply(function() {
                                $scope.hasPromotionalPrice = hasPromotionalPrice;
                            });
                        }
                        $("#has_promotional_price").on("change", function(e) {
                            hasPromotionalPrice = $(this).is(":checked");
                            $scope.updatePromotionalPrice();
                        });

                        // Product PromotionalPrices
                        $http({
                          url: window.baseUrl + "/_inc/ajax.php?p_id=" + product.p_id + "&type=PRODUCTPROMOTIONALPRICES",
                          method: "GET"
                        })
                        .then(function(response, status, headers, config) {
                            window.angular.forEach(response.data.promotional_prices, function(item, key) {
                                $scope.promotionalPriceSerial++;
                                var item = {
                                    'id' : $scope.promotionalPriceSerial,
                                ...
                                $scope.promotionalPriceArray.push(item);
                                $scope.initDatetimePicker();
                            });
                        }, function(data) {
                           window.swal("Oops!", "an error occured!", "error");
                        });
                        $scope.remotePromotionalPriceItem = function(index){
                            $scope.promotionalPriceArray.splice(index, 1);
                        }



                        var hasVariant =  $("#has_variant").is(":checked");
                        $scope.updateVariant = function() {
                            $scope.$apply(function() {
                                $scope.hasVariant = hasVariant;
                            });
                        }
                        $("#has_variant").on("change", function(e) {
                        ...
                        // Product Variants
                        $http({
                          url: window.baseUrl + "/_inc/ajax.php?p_id=" + product.p_id + "&type=PRODUCTVARIANTS",
                          method: "GET"
                        })
                        .then(function(response, status, headers, config) {
                            window.angular.forEach(response.data.variants, function(item, key) {
                                $scope.variantSerial++;
                                var item = {
                                    'id' : $scope.variantSerial,
                                    'name' : item.variant_name,
                                    'purchase_price_addition' : item.purchase_price_addition,
                                    'sell_price_addition' : item.sell_price_addition,
                                    'sort_order' : item.sort_order,
                                };
                                $scope.variantArray.push(item);
                            });
                        }, function(data) {
                           window.swal("Oops!", "an error occured!", "error");
                        });
                        $scope.remoteVariantItem = function(index){
                            $scope.variantArray.splice(index, 1);
                        }
                ...
                $scope.addPromotionalPriceItem = function() {
                    $scope.promotionalPriceSerial++;
                    var item = {
                        'id' : $scope.promotionalPriceSerial,
                        ...
                        'start_date' : '',
                        'end_date' : '',
                        'sort_order' : $scope.promotionalPriceSerial,
                        'status' : '1',
                    };
                    $scope.promotionalPriceArray.push(item);
                    $scope.initDatetimePicker();
                };
                $scope.addVariantItem = function() {
                    $scope.variantSerial++;
                    var item = {
                        'id' : $scope.variantSerial,
                        'name' : '',
                        'sell_price_addition' : '0',
                        'purchase_price_addition' : '0',
                        'sort_order' : $scope.variantSerial,
                    };
                    $scope.variantArray.push(item);
                };
                $scope.addImageItem = function() {
    ...
    return function($scope) {
        var uibModalInstance = $uibModal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $http({
                  url: window.baseUrl + "/_inc/template/payment_form.php?customer_id="+$scope.customerId+"&invoice_id="+$scope.invoiceId,
                  method: "GET"
                })
                .then(function(response, status, headers, config) {
                    ...
                    setTimeout(function() {
                        ...
                        if (undefined != $scope.pmethodId && $scope.pmethodId) {
                            // console.log($scope.pmethodId);
                            // console.log($("#pmethod_"+$scope.pmethodId).data('code'));
                            // console.log($("#pmethod_"+$scope.pmethodId).data('name'));
                            $scope.selectPaymentMethod($scope.pmethodId, $("#pmethod_"+$scope.pmethodId).data('code'), $("#pmethod_"+$scope.pmethodId).data('name'));
                        }
                    }, 500);
                }, function(response) {
                   window.swal("Oops!", response.data.errorMsg, "error");
                ...
                $scope.selectPaymentMethod = function(pmethodId,pmethodCode,pmethodName) {
                    $(document).find("body").addClass("overlay-loader");
    ...
    return function (invoice) {
        var uibModalInstance = $uibModal.open({
            ...
            template:   "<div id=\"data-modal\" class=\"modal-inner\">" +
                            ...
                            "<div class=\"modal-footer\" style=\"text-align:center;\">" +
                                "<button onClick=\"window.printContent('data-modal', {headline:'<small>Printed on: "+window.formatDate(new Date())+"</small>',screenSize:'fullScreen'})\" class=\"btn btn-primary\"><span class=\"fa fa-fw fa-print\"></span> Print</button>" +
                                "<a href=\"barcode_print.php?invoice_id="+encodeURI(invoice.invoice_id)+"\" class=\"btn btn-info\" target=\"_blink\"><span class=\"fa fa-fw fa-barcode\"></span> Barcode Print</a>" +
                            "</div>" +
                        "</div>",
            controller: function ($scope, $uibModalInstance) {
                $http({
