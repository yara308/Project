...
$scope.shippingCosts = {
    ...
    'منطقةB': 3.50,
    'منطقةC': 5.00
    // ... يمكنك إضافة المزيد من المناطق والتكاليف هنا
};
   $scope.submitForm = function() {
        if (!$scope.selectedZone) {
         ...
         // $scope.calculateShipping();
          $scope.updateCart(); 
        // هنا، بعد التحقق من صحة البيانات، قم بتحويل المستخدم إلى صفحة أخرى
        //window.location.href = "home.php ";
	    console.log("addShippingToCart function is called");
    };
$scope.calculateShipping = function() {
    return $scope.shippingCosts[$scope.selectedZone] || 0; 
};

$scope.isDeliveryProductAdded = function() {
    var found = $scope.itemArray.find(function(item) {
        return item.id === 1684; // هذا هو معرف منتج التوصيل
    });
    return found !== undefined;
};
$scope.updateCart = function() {
//console.log("updateCart function started");

    if ($scope.isDeliveryProductAdded()) {
       // console.log("Delivery product already exists");
		// تحديث المنتج إذا كان موجودًا بالفعل
        for (var i = 0; i < $scope.itemArray.length; i++) {
            if ($scope.itemArray[i].id === 1684) {
                $scope.itemArray[i].price = 2; // يمكنك تحديد التكلفة بناءً على منطقة الشحن أو أي شروط أخرى هنا
                break;
            }
        }
    } else {
        ...
        $scope.addItemToInvoice(1684, 1, 0);
    }
};
    ...
    $scope.addItemToInvoice = function (id, qty, index) {
	 ...
	 if (id == 1684) {
        var isProductAdded = window._.find($scope.itemArray, function (item) { 
            return item.id == id; 
        });

        // إذا تم العثور على المنتج في المصفوفة، فهذا يعني أنه تمت إضافته مسبقًا.
        if (isProductAdded) {
            window.toastr.error("لا يمكن اضافه منتج التوصيل اكثر من مره");
            return false;
        }
    }
        ...
        var $queryString = "p_id=" + id + "&action_type=PRODUCTITEM&api_key=123456";
...
//$scope.updateCart();
                    ...
                    window.toastr.success("'"+item.name+"' تم اضافه المنتج!", "بنجاح!");
                }
 if (!$scope.isDeliveryProductAdded()) {
                $scope.updateCart();
            }
    ...
    $scope.DecreaseItemFromInvoice = function (id, qty) {
		...
		$scope.isShippingProductAdded = function() {
    return window._.some($scope.itemArray, function(item) {
        return item.id == 1684;
    });
}
    $scope.DecreaseItemFromInvoice = function (id, qty) {
 ...
 if (id == 1684) {
        window.toastr.error("لا يمكن حذف منتج التوصيل");
        return false;
    }
        if ($scope.isEditMode) {
...
$scope.addValues = function(value1, value2) {
    return parseFloat(value1) + parseFloat(value2);
};

$scope.calculateTotal = function() {
  // احصل على إجمالي سعر المنتجات
  var totalPrice = 0;
  angular.forEach($scope.itemArray, function(item) {
    totalPrice += item.quantity * item.price;
  });

  // احصل على تكلفة الشحن
  var shippingCost = $scope.calculateShipping();

  // قم بتجميع المبالغ
  var total = totalPrice + shippingCost;

  // اعرض المبلغ الإجمالي
  $scope.totalAmount1 = total;

};
$scope.getTotal = function() {
    return parseFloat($scope.totalAmount) + parseFloat($scope.calculateShipping());
};
    ...
    $scope.placeOrder = function() {
        ...
        $http({
            url:  "/_inc/" + actionUrl + "?action_type=CREATE",
            method: "POST",
            data: form.serialize(),
            cache: false,
            processData: false,
        ...
        then(function(response) {
            if (response.data.status == 'ok') {
                ...
                .then(function (willDelete) {
                    // if (willDelete) {
                        if (pmethodName == 'Kent') {
                            window.location = "/pp_checkout.php?reference_no="+response.data.reference_no;
                        } else {
                            window.location = "/success.php?reference_no="+response.data.reference_no;
                        }
                    // }
                });
            }
