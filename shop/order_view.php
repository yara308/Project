...
$order = $model->getQuotationInfo($reference_no);
if (!$order) {
	redirect('account.php?tab=orders');
}
$order_items = $model->getQuotationItems($reference_no);
$document->setTitle(trans('title_view_order'));
include('header.php');?>

<style type="text/css">
@media only print {
	.breadcrumb-area, .site-topbar, .header-area, .footer-top {
		display: none;
	}
}
.table td {
...
<div class="breadcrumb-area section-padding-1 border-top-2 border-bottom-2 pt-30 pb-30">
    <div class="container-fluid">
        <div class="breadcrumb-content text-center breadcrumb-center">
            <h2>Order &raquo; <span class="text-warning"><?php echo $reference_no;?></span></h2>
            <ul>
                <li>
