				...
				<tbody>
					<tr ng-repeat="items in itemArray" class="bg-gray">
						<td class="text-center w-10">
							...
							<input type="hidden" name="product-item['{{ items.id }}'][item_price]" value="{{ items.price }}">
							<input type="hidden" name="product-item['{{ items.id }}'][item_quantity]" value="{{ items.quantity }}">
							<input type="hidden" name="product-item['{{ items.id }}'][item_total]" value="{{ items.subTotal }}">
							{{ $index+1 }}
						</td>
						<td class="w-70">{{ items.name }}<span ng-show="items.variantSlug"> ({{ items.variantName }})</span> x  x{{ items.quantity }} {{ items.unitName }}</td>
