...
$invoice_id = isset($request->get['invoice_id']) && $request->get['invoice_id'] != 'undefined' && $request->get['invoice_id'] != 'null' ? (bool)$request->get['invoice_id'] : 0;
$customer_id = isset($request->get['customer_id']) && $request->get['customer_id'] ? $request->get['customer_id'] : 0;?>
		...
		<div class="col-lg-3 col-md-3 col-sm-3 bootboox-tab-menu bootboox-container p-0">
			<div class="list-group">
				<?php $inc = 0;foreach(get_pmethods() as $pmethod) :?>
					<a class="text-left list-group-item pmethod_item" id="pmethod_<?php echo $pmethod['pmethod_id']; ?>" href="javascript:void(0)" <?php echo $inc == 0 && !$invoice_id ? 'ng-init="selectPaymentMethod('.$pmethod['pmethod_id'].',\''.$pmethod['code_name'].'\',\''.$pmethod['name'].'\')"' : null;?> ng-click="selectPaymentMethod('<?php echo $pmethod['pmethod_id']; ?>', '<?php echo $pmethod['code_name']; ?>', '<?php echo $pmethod['name']; ?>')" onClick="return false;" data-code="<?php echo $pmethod['code_name'];?>" data-name="<?php echo $pmethod['name'];?>"><span class="fa fa-fw fa-angle-double-right"></span> <b><?php echo $pmethod['name']; ?>
					...
					<tbody>
						<tr ng-repeat="items in itemArray" class="bg-gray">
							<td class="text-center w-10">
								...
								<input type="hidden" name="product-item['{{ items.id }}'][variant_slug]" value="{{ items.variantSlug }}">
								<input type="hidden" name="product-item['{{ items.id }}'][variant_name]" value="{{ items.variantName }}">
							...
							<td class="text-right w-20">{{ items.subTotal | curConvert:conValue:currencyCode | formatDecimal:2 }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							...
							<td class="text-right w-40">{{ totalAmount | curConvert:conValue:currencyCode | formatDecimal:2 }}</td>
						</tr>
						<tr>
							<th class="text-right w-60"  colspan="2">
						...
						<tr>
							...
							<td class="text-right w-40">{{ totalPayable | curConvert:conValue:currencyCode | formatDecimal:2 }}</td>
						</tr>
						<tr>
							<th class="text-right w-60" colspan="2">
						...
						<tr>
							...
							<td class="text-right w-40">{{ (totalPayable | curConvert:conValue:currencyCode) < paidAmount ? 0 : (totalPayable | curConvert:conValue:currencyCode) - paidAmount | formatDecimal:2 }}</td>
						</tr>
						<tr>
							<th class="text-right w-60" colspan="2">
								<?php echo trans('label_balance'); ?>
							</th>
							<td class="text-right w-40">{{ totalPayable < paidAmount ? paidAmount - totalPayable  : 0 | curConvert:conValue:currencyCode | formatDecimal:2 }}</td>
						</tr>
