...
<?php 
$customer_name = get_the_customer($order['customer_id'],'customer_name');
$pmethod_details1 = get_the_customer($order['customer_id'],'pmethod_details');
$customer_address = get_the_customer($order['customer_id'],'customer_address');
	...
	<table class="table table-bordered table-condensed">
		<tbody>
			...
			<tr>
				...
				<td style="width:50%;">
					<?php echo trans('label_invoice_id');?>: <span class="text-primary"><?php echo $order['invoice_id'] ? $order['invoice_id'] : 'N/A';?></span>
				</td>
			</tr>
			<tr>
				...
				<td style="width:50%;vertical-align:top;">
					...
					<address class="mb-10">
						<?php $address = $order['address'] ? json_decode($order['address']) : array() ;?>
					...
					<address class="mb-10">
						<?php $pmethod_details = $order['pmethod_details'] ? json_decode($order['pmethod_details']) : array();?>
						<?php foreach ($pmethod_details as $key => $pm):?>
							<span><?php echo ucfirst(str_replace(array('_'), ' ', $key));?>: <?php echo $pm;?></span><br>
						<?php endforeach;?>
						<?php echo $pmethod_details1;?>
					</address>
				</td>
			</tr>
		</tbody>
					...
					<tbody>
						<?php $inc=1;foreach ($order_items as $product) : ?>
							<tr>
								...
								<td>
									<?php echo $product['item_name']; ?><?php echo $product['variant_name'] ? ' ('.$product['variant_name'].')' : null;?> x <?php echo currency_format($product['item_quantity']); ?> <?php echo $product['unitName']; ?>
								</td>
								<td class="text-right">
									<?php echo currency_format($product['item_price'], $order['currency']); ?>
								</td>
								<td class="text-right">
									<?php echo currency_format($product['item_tax']); ?>
								</td>
								<td class="text-right">
									<?php echo currency_format($product['item_total'], $order['currency']); ?>
								</td>
							</tr>
						<?php $inc++;endforeach; ?>
					</tbody>
					<tfoot>
						<tr>
							...
							<td class="w-20 text-right">
								<?php echo currency_format($order['subtotal']+$order['item_tax'], $order['currency']); ?>
							</td>
						</tr>
						<tr>
							<td class="text-right" colspan="4">
						...
						<tr>
							...
							<td class="w-20 text-right" style="font-weight:bold">
								<?php echo currency_format($order['payable_amount'], $order['currency']); ?>
							</td>
						</tr>
