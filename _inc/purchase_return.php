...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'RETURN')
{
  try {
    ...
    foreach ($items as $item) 
    {
      ...
      $purchase_price_addition = get_the_variant($item_id, $item_variant_slug, 'purchase_price_addition');
      $item_price = $invoice_item['item_purchase_price'] + (float)$purchase_price_addition;
      ...
      $statement = db()->prepare("INSERT INTO `purchase_return_items` SET `store_id` = ?, `invoice_id` = ?, `item_id` = ?, `item_name` = ?, `item_variant_name` = ?, `item_variant_slug` = ?, `item_quantity` = ?, `item_price` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `item_total` = ?");
      $statement->execute(array($store_id, $invoice_id, $item_id, $item_name, $item_variant_name, $item_variant_slug, $item_quantity, $item_price, $item_tax, $cgst, $sgst, $igst, $item_total));
    };
