...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATE')
{
  try {
    ...
    // Sparrow
    $setting = $request->post['setting']['sparrow'];
    $statement = db()->prepare("UPDATE `sms_setting` SET `api_id` = ?, `sender_id` = ? WHERE `type` = ?");
    $statement->execute(array($setting['token'], $setting['identity'], 'Sparrow'));
