...
function validate_request_data($request) 
{
 ...
 // if (!validateString($request->post['reference_no'])) {
  ...
  // Validate tax amount
  if (!is_numeric($request->post['total-tax-amount'])) {
...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'CREATE')
{
  try {
    ...
    validate_quotation_items($request->post['products']);
        ...
        'formatter' => function($d, $row){
          $o = '<div class="btn-group btn-block">
            ...
            if ($row['status'] != 'complete' && user_group_id() == 1 || has_permission('access', 'update_quotation')) {
              $o .= '<li><a href="quotation.php?box_state=open&reference_no='.$row['reference_no'].'"> '.trans('button_edit').'</a></li>';
            }
            ...
            if (user_group_id() == 1 || has_permission('access', 'send_email')) {
              $o .= '<li><a class="email-the-quotation" href="#">'.trans('button_send_email').'</a></li>';
            }
          $o .= '</ul></div>';
          return $o;
        }
