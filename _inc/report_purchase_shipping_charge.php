...
$table = "(SELECT `purchase_info`.*, `purchase_price`.`shipping_amount` AS `shipping_charge` FROM `purchase_info` LEFT JOIN `purchase_price` ON (`purchase_info`.`invoice_id`=`purchase_price`.`invoice_id`) WHERE {$where_query}) as purchase_info";
