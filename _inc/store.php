    ...
    if (!empty($request->post['product'])) {
      foreach ($request->post['product'] as $product_id) {
        ...
        //--- Brand to store ---//
          ...
          $statement = db()->prepare("SELECT * FROM `brand_to_store` WHERE `store_id` = ? AND `brand_id` = ?");
          $statement->execute(array($store_id, $product_info['brand_id']));
          $brand = $statement->fetch(PDO::FETCH_ASSOC);
          if (!$brand) {
            $statement = db()->prepare("INSERT INTO `brand_to_store` SET `brand_id` = ?, `store_id` = ?");
            $statement->execute(array((int)$product_info['brand_id'], (int)$store_id));
          }

        //--- Unit to store ---//

          if ($product_info['unit_id']) {
            $statement = db()->prepare("SELECT * FROM `unit_to_store` WHERE `store_id` = ? AND `uunit_id` = ?");
            $statement->execute(array($store_id, $product_info['unit_id']));
            $unit = $statement->fetch(PDO::FETCH_ASSOC);
            if (!$unit) {
              $statement = db()->prepare("INSERT INTO `unit_to_store` SET `uunit_id` = ?, `store_id` = ?");
              $statement->execute(array((int)$product_info['unit_id'], (int)$store_id));
            }
          }

        //--- Create product link ---//

          $statement = db()->prepare("INSERT INTO `product_to_store` SET `product_id` = ?, `store_id` = ?, `purchase_price` = ?, `sell_price` = ?, `alert_quantity` = ?, `sup_id` = ?, `brand_id` = ?, `box_id` = ?, `taxrate_id` = ?, `tax_method` = ?, `preference` = ?, `e_date` = ?, `p_date` = ?, `status` = ?, `sort_order` = ?");
          $statement->execute(array($product_id, $store_id, $product_info['purchase_price'], $product_info['sell_price'], $product_info['alert_quantity'], $product_info['sup_id'], $product_info['brand_id'], $product_info['box_id'], $product_info['taxrate_id'], $product_info['tax_method'], $product_info['preference'], $product_info['e_date'], date('Y-m-d'), $product_info['status'], $product_info['sort_order']));
      }
    }
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATE')
{
  try {
    ...
    $about = $request->post['about_text'];
    $term_condition = $request->post['term_condition_text'];
    $refund_policy = $request->post['refund_policy_text'];
    $statement = db()->prepare("UPDATE shop SET `term_condition` = ?, `refund_policy` = ?, `short_description` = ?, `about` = ? WHERE `id` = ?");
    $statement->execute(array($term_condition, $refund_policy, $short_description, $about, 1));
