...
if (isset($request->get['item_name']) && $request->get['item_name']) {
    $item_name = $request->get['item_name'];
    $table = "(SELECT selling_info.* FROM `selling_info` 
    LEFT JOIN selling_item ON (selling_info.invoice_id = selling_item.invoice_id) 
    WHERE {$where_query} AND selling_item.item_name LIKE '%{$item_name}%') as selling_info";
} else {
...
$columns = array(
    ...
    array(
        ...
        'formatter' => function($d, $row){
            ...
            return '<a href="pos.php?invoice_id='.$row['invoice_id'].'" class="btn btn-sm btn-block btn-primary" title="'.trans('button_edit').'" data-loading-text="..."><span class="fa fa-pencil"></span></a>'; 
        }
    ),
    array(
        'db' => 'invoice_id',
