...
function validate_request_data($request) 
{
  ...
  // Validate supplier
  if (!isset($request->post['supplier']) || empty($request->post['supplier'])) {
    throw new Exception(trans('error_supplier'));
  }
...
$columns = array(
  ...
  array( 
    ...
    'formatter' => function($d, $row) {
      ...
      return get_product_supplier_html($row['p_id']);
    }
  ),
  ...
  array( 
    ...
    'formatter' => function($d, $row) {
      if ($row['status'] && $row['p_type'] != 'service') {
        return '<a href="purchase.php?box_state=open&sup_id='.get_product_first_supplier_id($row['p_id']).'&p_code='.$row['p_code'].'" class="btn btn-block btn-sm btn-success" title="'.trans('button_purchase_product').'"><i class="fa fa-shopping-cart"></i></a>';
      }
