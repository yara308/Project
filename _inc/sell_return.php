  ...
  try {
    ...
    foreach ($items as $item) 
    {
      ...
      {                
        ...
        if (!$purchase_item) 
        {
          throw new Exception('The product: '.$product['p_name'].' was not found in purchase register');
        }        
      ...
      $item_variant_name = $invoice_item['item_variant_name'];
      $item_variant_slug = $invoice_item['item_variant_slug'];
      $sell_price_addition = get_the_variant($item_id, $item_variant_slug, 'sell_price_addition');
      $item_price = $invoice_item['item_price'] + (float)$sell_price_addition;
      ...
      $statement = db()->prepare("INSERT INTO `return_items` SET `store_id` = ?, `invoice_id` = ?, `item_id` = ?, `item_name` = ?, `item_variant_name` =?, `item_variant_slug` = ?, `item_quantity` = ?, `item_price` = ?, `item_purchase_price` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `item_total` = ?, `created_at` = ?");
      $statement->execute(array($store_id, $invoice_id, $item_id, $item_name, $item_variant_name, $item_variant_slug, $item_quantity, $item_price, $item_purchase_price, $item_tax, $cgst, $sgst, $igst, $item_total, date_time()));
    };
    ...
    if ($return_amount > 0) {
      ...
      $statement = db()->prepare("INSERT INTO `payments` SET `type` = ?,  `is_profit` = ?, `is_hide` = ?, `store_id` = ?, `invoice_id` = ?, `reference_no` = ?, `capital` = ?, `amount` = ?, `created_by` = ?, `created_at` = ?");
      $statement->execute(array('return', $is_profit, $is_hide, $store_id, $invoice_id, $reference_no, -$capital, -$return_amount, $user_id, date_time()));
    ...
    if ($balance > 0) {
      $statement = db()->prepare("INSERT INTO `payments` SET `type` = ?, `store_id` = ?, `invoice_id` = ?, `note` = ?, `pos_balance` = ?, `created_by` = ?, `created_at` = ?");
