 ...
 *===================
 */
$where_query = "selling_info.inv_type != 'due_paid'";
if ((user_group_id() != 1 && !has_permission('access', 'read_global_sell_report')) || store_filter()) {
  $where_query .= " AND selling_info.store_id = '{$store_id}'";
}
...
// DB table to use
$table = "(SELECT @sl:=@sl+1 AS sl, selling_info.store_id, selling_info.invoice_id, selling_info.created_at, selling_item.id, selling_item.item_id, selling_item.item_name, selling_item.item_variant_name, SUM(selling_item.item_quantity) as total_item, SUM(selling_item.item_discount) as discount, SUM(selling_item.item_tax) as tax, SUM(selling_item.item_purchase_price) as purchase_price, SUM(selling_item.item_total) as sell_price FROM selling_item 
  ...
  WHERE $where_query
  GROUP BY selling_item.item_variant_slug, selling_item.store_id
...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'item_name',
      'formatter' => function( $d, $row ) {
        $variant_name = $row['item_variant_name'] && $row['item_variant_name'] != 'NULL' ? '('.$row['item_variant_name'].')' : '';
        return '<a href="product.php?p_id=' . $row['item_id'] . '&p_name=' . $row['item_name'] . '">' . $row['item_name'].' '.$variant_name.'</a>';
      }
    ),
    array( 
      'db' => 'total_item',  
