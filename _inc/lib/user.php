...
class User 
{
	...
	public function __construct($registry)
	{
		...
		if (count(get_expired_products()) > 0) {
			switch (get_preference('remove_expired')) {
				case 'yes': //Remove from stock automatically

					foreach (get_expired_products() as $product) {
						if ($product['quantity_in_stock'] <= 0) { continue; }
						$store_id = $product['store_id'];
						$item_id = $product['product_id'];
						$statement = $this->db->prepare("UPDATE `purchase_item` SET `status` = ? WHERE `store_id` = ? AND `item_id` = ?");
						$statement->execute(array('sold', $store_id, $item_id));

						$statement = $this->db->prepare("UPDATE `product_to_store` SET `quantity_in_stock` = ? WHERE `store_id` = ? AND `product_id` = ?");
						$statement->execute(array(0, $store_id, $item_id));
					}

					break;
				
				case 'delete': //Remove from stock automatically and delete the product
					...
					foreach (get_expired_products() as $product) {
						...
						$statement = $this->db->prepare("UPDATE `purchase_item` SET `status` = ? WHERE `store_id` = ? AND `item_id` = ?");
						$statement->execute(array('sold', $store_id, $item_id));
				        $product_model->deleteProduct($item_id);
				    }
	...
	public function getBelongsStore($user_id = null)
	{
		...
		$statement = $this->db->prepare("SELECT `s`.* FROM `stores` s LEFT JOIN `user_to_store` u2s ON (`s`.`store_id` = `u2s`.`store_id`) WHERE `user_id` = ? GROUP BY `store_id`");
	...
	public function countBelongsStore($user_id = null)
	{
		...
		$statement = $this->db->prepare("SELECT * FROM `user_to_store` WHERE `user_id` = ? GROUP BY `store_id`");
		$statement->execute(array($user_id));
