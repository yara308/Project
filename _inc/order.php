...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATESTATUS')
{
  try {
    ...
    if (user_group_id() != 1 && !has_permission('access', 'updadte_approve_status')) {
    ...
    $statement = db()->prepare("UPDATE `quotation_info` SET `is_approve` = ? WHERE `info_id` = ?");
...
$columns = array(
    ...
    'formatter' => function($d, $row) {
      ...
      $check_is_approve = $row['is_approve'] == 1 ? ' checked=checked ' : '';
      $html = '<div class="onoffswitch-small" id="'.$row['info_id'].'">';
      $html .= '<input type="checkbox" id="myonoffswitch'.$row['info_id'].'" class="onoffswitch-small-checkbox" name="is_approve"'.$check_is_approve.'data-url="'.root_url().'/_inc/order.php?action_type" data-datatable="order-order-list">';
      $html .= '<label for="myonoffswitch'.$row['info_id'].'" class="onoffswitch-small-label">';
      $html .= '<span class="onoffswitch-small-inner approve-switch"></span>';
  ...
  array(
      ...
      'formatter' => function($d, $row) {
        $o = '<div class="btn-group btn-block">
          ...
          if (!($row['status'] == 'complete' && $row['invoice_id'] == true) && user_group_id() == 1 || has_permission('access', 'update_order')) {
            $o .= '<li><a href="order_edit.php?reference_no='.$row['reference_no'].'"> '.trans('button_edit').'</a></li>';
          }
          if ($row['is_approve'] == 1 && !($row['status'] == 'complete' && $row['invoice_id'] == true) && user_group_id() == 1 || has_permission('access', 'create_sell_invoice')) {
            $o .= '<li><a href="pos.php?qref='.$row['reference_no'].'">'.trans('button_create_sell').'</a></li>';
          }
          if (!($row['status'] == 'complete' && $row['invoice_id'] == true) && user_group_id() == 1 || has_permission('access', 'delete_order')) {
