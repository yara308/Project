...
class ModelOrder extends Model 
{
    public function updateOrder($reference_no, $request, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            $statement->execute(array($reference_no, $store_id, $sup_id, $category_id, $brand_id, $product_id, $product_name, $variant_slug, $variant_name, $item_purchase_price, $product_price, $product_discount, $tax, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id));
        }
    ...
    public function getOrderItems($reference_no, $store_id = null) 
    {
        ...
        foreach ($rows as $row) {
            ...
            $array[$i]['variant_slug'] = '';
            $array[$i]['variant_name'] = '';
            $array[$i]['sell_price_addition'] = 0;
            $the_variant = get_the_variant($row['item_id'], $row['item_variant_slug']);
            if ($the_variant) {
                $array[$i]['variant_id'] = $the_variant['variant_id'];
                $array[$i]['variant_slug'] = $the_variant['variant_slug'];
                $array[$i]['variant_name'] = $the_variant['variant_name'];
                $array[$i]['purchase_price_addition'] = $the_variant['purchase_price_addition'];
                $array[$i]['sell_price_addition'] = $the_variant['sell_price_addition'];
            }
            if (!isset($array[$i]['sell_price_addition'])) {
                $array[$i]['purchase_price_addition'] = 0;
                $array[$i]['sell_price_addition'] = 0;
            }
            $array[$i]['variant'] = $the_variant;
