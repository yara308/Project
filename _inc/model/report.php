...
class ModelReport extends Model 
{
	...
	public function getTopProfitableProducts($from, $to, $limit = 3, $store_id = null)
	{
		$store_id = $store_id ? $store_id : store_id();
		$where_query = "`selling_info`.`store_id` = '$store_id'";
		$where_query .= date_range_filter($from, $to);
		$statement = $this->db->prepare("SELECT `selling_info`.`store_id`, `selling_info`.`created_at`, `selling_item`.`item_name`, SUM(`selling_item`.`profit`) as profit FROM `selling_info` 
			LEFT JOIN `selling_item` ON (`selling_info`.`invoice_id` = `selling_item`.`invoice_id`)
			WHERE $where_query
			GROUP BY `selling_item`.`item_id` ORDER BY `profit` 
			DESC LIMIT $limit");
		$statement->execute(array());
		return $statement->fetchAll(PDO::FETCH_ASSOC);
	}
