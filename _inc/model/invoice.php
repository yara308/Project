...
class ModelInvoice extends Model
{
    public function putOrderOnHold($request, $ref_no, $store_id = null)
    {
        ...
        foreach ($product_items as $product) 
        {
            ...
            $statement = $this->db->prepare("INSERT INTO `holding_item` (ref_no, store_id, item_id, item_variant_slug, item_variant_name, category_id, brand_id, sup_id, item_name, notes, item_price, item_discount, item_tax, tax_method, taxrate_id, tax, gst, cgst, sgst, igst, item_quantity, item_total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($ref_no, $store_id, $product_id, $item_variant_slug, $item_variant_name, $category_id, $brand_id, $sup_id, $product_name, $product_notes, $product_price, $product_discount, $tax, $tax_method, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total));
        }
        $statement = $this->db->prepare("INSERT INTO `holding_info` (store_id, order_title, ref_no, customer_id, customer_mobile, currency, invoice_note, total_items, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($store_id, $order_title, $ref_no, $customer_id, $customer_mobile, $currency, $invoice_note, $total_items, $user_id, $created_at));
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        $invoice_id = isset($request->post['invoice-id']) && $request->post['invoice-id'] ? $request->post['invoice-id'] : '';
        $invoice = get_the_invoice($invoice_id);
        if ($invoice) {
          $due = $invoice['due'];
          $the_invoice_paid_amount = $invoice['paid_amount'];
          // $paid_amount = $invoice['paid_amount'];
          $invoice_items = get_invoice_items($invoice_id);
          foreach ($invoice_items as $item) {
            $item_quantity = $item['item_quantity'];
            $statement = $this->db->prepare("UPDATE `product_to_store` SET `quantity_in_stock` = `quantity_in_stock`+{$item_quantity} WHERE `store_id` = ? AND `product_id` = ?");
            $statement->execute(array($store_id, $item['item_id']));

            $statement = $this->db->prepare("UPDATE `purchase_item` SET `total_sell` = `total_sell`-{$item_quantity}, `status` = 'active' WHERE `store_id` = ? AND `invoice_id` = ? AND `item_id` = ?");
            $statement->execute(array($store_id, $item['purchase_invoice_id'], $item['item_id']));
          }

          $update_due = $this->db->prepare("UPDATE `customer_to_store` SET `balance` = `balance`-{$paid_amount} WHERE `store_id` = ? AND `customer_id` = ?");
          $update_due->execute(array($store_id, $customer_id));

          $update_due = $this->db->prepare("UPDATE `customer_to_store` SET `due` = `due`-{$due} WHERE `store_id` = ? AND `customer_id` = ?");
          $update_due->execute(array($store_id, $customer_id));

          $statement = $this->db->prepare("DELETE FROM `selling_info` WHERE `invoice_id` = ?");
          $statement->execute(array($invoice_id));

          $statement = $this->db->prepare("DELETE FROM `selling_item` WHERE `invoice_id` = ?");
          $statement->execute(array($invoice_id));

          $statement = $this->db->prepare("DELETE FROM `selling_price` WHERE `invoice_id` = ?");
          $statement->execute(array($invoice_id));

          $statement = $this->db->prepare("DELETE FROM `customer_transactions` WHERE `ref_invoice_id` = ?");
          $statement->execute(array($invoice_id));

          $statement = $this->db->prepare("DELETE FROM `payments` WHERE `invoice_id` = ?");
          $statement->execute(array($invoice_id));

          $statement = $this->db->prepare("DELETE FROM `sell_logs` WHERE `ref_invoice_id` = ?");
          $statement->execute(array($invoice_id));

          // Withdraw
          if (($account_id = store('deposit_account_id')) && $the_invoice_paid_amount > 0) {
            $ref_no = unique_transaction_ref_no('withdraw');
            $statement = $this->db->prepare("SELECT `category_id` FROM `expense_categorys` WHERE `sell_edit_debit` = ?");
            $statement->execute(array(1));
            $category = $statement->fetch(PDO::FETCH_ASSOC);
            $exp_category_id = $category['category_id'];
            $title = 'Debit for Sell Editing';
            $withdraw_details = 'Customer name: ' . get_the_customer($customer_id, 'customer_name');
            $image = 'NULL';
            $withdraw_amount = $the_invoice_paid_amount;
            $transaction_type = 'withdraw';

            $statement = $this->db->prepare("INSERT INTO `bank_transaction_info` (store_id, account_id, exp_category_id, ref_no, invoice_id, transaction_type, title, details, image, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($store_id, $account_id, $exp_category_id, $ref_no, $invoice_id, $transaction_type, $title, $withdraw_details, $image, $user_id, date_time()));
        ...
        foreach ($product_items as $product) 
        {
            ...
            $brand_id = $product_info['brand_id'];
            $sup_id = $product['sup_id'];
            $gender_for = $product_info['gender_for'];
            $product_name = $product['item_name'];
            $product_notes = $product['notes'];
            $product_quantity = $product['item_quantity'];
            ...
            while ($quantity_exist > 0) {
                ...
                $where_query = '';
                if ($item_variant_slug) {
                    $where_query =  " AND `item_variant_slug` = '{$item_variant_slug}'";
                }

                $statement = $this->db->prepare("SELECT * FROM `purchase_item` WHERE `store_id` = ? AND `item_id` = ?{$where_query} AND `status` = ? AND `item_quantity` > `total_sell`");
                $statement->execute(array($store_id, $product_id, 'active'));
                $purchase_item = $statement->fetch(PDO::FETCH_ASSOC);
                if (!$purchase_item) {
                    $statement = $this->db->prepare("UPDATE `purchase_item` SET `status` = ? WHERE `store_id` = ? AND `item_id` = ?{$where_query} AND `status` = ?");
                ...
                $statement = $this->db->prepare("UPDATE `purchase_item` SET `total_sell` = `total_sell` + $sell_quantity WHERE `store_id` = ? AND `item_id` = ?{$where_query} AND `status` = ?");
                $statement->execute(array($store_id, $product_id, 'active'));
                $statement = $this->db->prepare("SELECT * FROM `purchase_item` WHERE `store_id` = ? AND `item_id` = ?{$where_query} AND `status` = ?");
                $statement->execute(array($store_id, $product_id, 'active'));
                $purchase_item = $statement->fetch(PDO::FETCH_ASSOC);
                ...
                $inc++;
                if ($inc > 1000) {
                    throw new Exception(trans('error_stockx'));
                }
            }
            if (get_the_customer($customer_id, 'customer_state') == get_preference('business_state')) {
                $cgst = $tax / 2;
            ...
            if ($product['p_type'] == 'service') { 
                $profit = ($product_price - ($product_info['purchase_price']+$product_discount)) * $sell_quantity;
            } else {
                $profit = ($product_price - ($purchase_item['item_purchase_price']+$product_discount)) * $sell_quantity;
            }
            
            if ($tax_method == 'inclusive') {
                $profit = $profit - $item_tax;
            }
            $statement = $this->db->prepare("INSERT INTO `selling_item` (invoice_id, store_id, item_id, item_variant_slug, item_variant_name, category_id, brand_id, sup_id, gender_for, item_name, notes, item_purchase_price, item_price, item_discount, item_tax, tax_method, taxrate_id, tax, gst, cgst, sgst, igst, item_quantity, item_total, purchase_invoice_id, profit) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($invoice_id, $store_id, $product_id, $item_variant_slug, $item_variant_name, $category_id, $brand_id, $sup_id, $gender_for, $product_name, $product_notes, $item_purchase_price, $product_price, $product_discount, $tax, $tax_method, $taxrate_id, $taxrate, $taxrate, $cgst, $sgst, $igst, $product_quantity, $product_total, $purchase_invoice_id, $profit));
        ...
        $statement = $this->db->prepare("INSERT INTO `selling_info` (invoice_id, store_id, customer_id, customer_mobile, currency, invoice_note, total_items, payment_status, is_installment, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($invoice_id, $store_id, $customer_id, $customer_mobile, $currency, $invoice_note, $total_items, $payment_status, $is_installment_order, $user_id, $created_at));
        ...
        $statement = $this->db->prepare("INSERT INTO `sell_logs` (customer_id, reference_no, type, pmethod_id, description, amount, store_id, ref_invoice_id, created_by, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)");
        ...
        // // Send SMS through Mobile SMS Gateway
        // require_once(DIR_INCLUDE.'/mobile-sms-gateway-api/autoload.php');
        // $apiClient = new SMSGatewayApi(AUTH_KEY, SERVER);
        // @$apiClient->sendSMS('01737346122', "Thank you for purchasing from ".store('name').". Invoice ID: ".$invoice_id.", Payable: ".currency_format($payable_amount).", Paid: ".currency_format($paid_amount), 'ARS-L22', 2);
    ...
    function duePaid($data, $store_id = null)
    {
        ...
        if ($paid_amount > 0 && $discount_amount <= 0) {
          $statement = $this->db->prepare("INSERT INTO `payments` (type, store_id, invoice_id, pmethod_id, capital, amount, details, note, total_paid, pos_balance, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($discount_amount > 0) {
          $statement = $this->db->prepare("INSERT INTO `payments` (type, store_id, invoice_id, amount, details, note, total_paid, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
          ...
          $statement = $this->db->prepare("UPDATE `selling_price` SET `discount_amount` = `discount_amount`+$discount_amount, `payable_amount` = `payable_amount`-$discount_amount WHERE `invoice_id` = ? AND `store_id` = ?");
          $statement->execute(array($invoice_id, $store_id));
        }

        // Checkout status
        $statement = $this->db->prepare("UPDATE `selling_info` SET `checkout_status` = ? WHERE `invoice_id` = ? AND `store_id` = ?");
        $statement->execute(array(1, $invoice_id, $store_id));

        // Add Paid Amount
        $statement = $this->db->prepare("UPDATE `selling_price` SET `paid_amount` = `paid_amount`+$paid_amount, `due_paid` = `due_paid`+$paid_amount, `due` = ? WHERE `invoice_id` = ? AND `store_id` = ?");
        $statement->execute(array($due, $invoice_id, $store_id));

        // Update payment status
        if ($due <= 0) {
          $statement = $this->db->prepare("UPDATE `selling_info` SET `payment_status` = ? WHERE `invoice_id` = ? AND `store_id` = ?");
          $statement->execute(array('paid', $invoice_id, $store_id));
        }

        // Update customer due
        $statement = $this->db->prepare("UPDATE `customer_to_store` SET `due` = `due` - {$paid_amount}  WHERE `store_id` = ? AND `customer_id` = ?");
        $statement->execute(array($store_id, $customer_id));
        ...
        if (($account_id = store('deposit_account_id')) && $paid_amount > 0) {
          $ref_no = unique_transaction_ref_no();
          $statement = $this->db->prepare("SELECT `source_id` FROM `income_sources` WHERE `for_due_collection` = ?");
          $statement->execute(array(1));
          $source = $statement->fetch(PDO::FETCH_ASSOC);
          $source_id = $source['source_id'];
        ...
        if ($balance > 0) {
          $statement = $this->db->prepare("UPDATE `selling_price` SET `balance` = ? WHERE `store_id` = ? AND `invoice_id` = ?");
    ...
    public function getInvoiceItemsHTML($invoice_id, $store_id = null)
    {
        ...
        $rows = $statement->fetchAll(PDO::FETCH_ASSOC);
        $i = 0;
        $html = '<div class="panel-group mb-0" id="accordion'.md5($invoice_id).'" role="tablist" aria-multiselectable="true">';
        $html .= '<div class="panel panel-default bg-gray p-0">';
        $html .= '<div class="panel-heading p-5" role="tab" id="headingOne'.md5($invoice_id).'">';
        $html .= '<h4 class="panel-title bg-info">';
        $html .= '<a role="button" data-toggle="collapse" data-parent="#accordion'.md5($invoice_id).'" href="#collapseOne'.md5($invoice_id).'" aria-expanded="true" aria-controls="collapseOne'.md5($invoice_id).'">';
        $html .= 'click for details';
        $html .= '</a>';
        $html .= '</h4>';
        $html .= '</div>';
        $html .= '<div id="collapseOne'.md5($invoice_id).'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne'.md5($invoice_id).'">';
        $html .= '<div class="panel-body p-0">';
            ...
            $total = 0;
            foreach ($rows as $row) {
                $item = get_the_product($row['item_id']);
                $unit = isset($item['unit_id']) ? get_the_unit($item['unit_id'], 'unit_name') : '';
                $vname = get_the_variant($row['item_id'], $row['item_variant_slug'],'variant_name');
                $variant_name = $vname ? '('.$vname.')' : null;
                $html .= '<tr class="bg-success">';
                $html .= '<td class="text-center" style="padding:0 2px;">' . $row['item_name'] . $variant_name . '</td>';
                $html .= '<td class="text-right" style="padding:0 2px;">' . currency_format($row['item_price']) . '</td>';
                $html .= '<td class="text-center" style="padding:0 2px;">' . currency_format($row['item_quantity']) . ' ' . $unit . '</td>';
