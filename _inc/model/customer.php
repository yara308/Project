...
class ModelCustomer extends Model 
{
	...
	{
		$due = (float)$data['due'] - (float)$data['credit_balance'];
		$gtin = isset($data['gtin']) ? $data['gtin'] : '';
		$customer_state = isset($data['customer_state']) ? $data['customer_state'] : '';
		$pmethod_details = 'COD';
    	$statement = $this->db->prepare("INSERT INTO `customers` (customer_name, dob, customer_email, customer_mobile, customer_sex, customer_age, gtin, customer_address, customer_city, customer_state, customer_country, created_at , address ,pmethod_details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    	$statement->execute(array($data['customer_name'], date('Y-m-d',strtotime($data['dob'])), $data['customer_email'], $data['customer_mobile'], $data['customer_sex'], $data['customer_age'], $gtin, $data['customer_address'], $data['customer_city'], $customer_state, $data['customer_country'], date_time(), $data['customer_address'], $pmethod_details));
    	$customer_id = $this->db->lastInsertId();

    	if ($due > 0) {
    		$store_id = store_id();
    		$invoice_id = generate_invoice_id('sell');
    		$customer_mobile = get_the_customer($customer_id, 'customer_mobile');
    		$statement = $this->db->prepare("INSERT INTO `selling_item` (invoice_id, store_id, item_id, category_id, sup_id, item_name, item_purchase_price, item_price, item_tax, tax_method, tax, gst, cgst, sgst, igst, item_quantity, item_total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($invoice_id, $store_id, 1, 1, 1, 'Customer Initial Due', $due, $due, 0, 'inclusive', 0, 0, 0, 0, 0, 1, $due));

            $statement = $this->db->prepare("INSERT INTO `selling_info` (invoice_id, store_id, customer_id, customer_mobile, total_items, payment_status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
	        $statement->execute(array($invoice_id, $store_id, $customer_id, $customer_mobile, 1, 'due', user_id(), date_time()));
	        
	        $statement = $this->db->prepare("INSERT INTO `selling_price` (invoice_id, store_id, subtotal, payable_amount, paid_amount, due) VALUES (?, ?, ?, ?, ?, ?)");
	        $statement->execute(array($invoice_id, $store_id, $due, $due, 0, $due));
    	}
		...
		if (isset($data['customer_store'])) {
			...
			foreach ($data['customer_store'] as $store_id) {
				if ($inc == 0) {
					$statement = $this->db->prepare("INSERT INTO `customer_to_store` SET `customer_id` = ?, `due` = ?, `store_id` = ?");
					$statement->execute(array((int)$customer_id, (float)$due, (int)$store_id));
				} else {
