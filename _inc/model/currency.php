...
class ModelCurrency extends Model 
{
	...
	{
	   	$statement = $this->db->prepare("INSERT INTO `currency` (title, code, value, symbol_left, symbol_right, decimal_place, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
    	$statement->execute(array($data['title'], $data['code'], $data['value'], $data['symbol_left'], $data['symbol_right'], $data['decimal_place'], date('Y-m-d H:i:s')));
	...
	{ 
    	$statement = $this->db->prepare("UPDATE `currency` SET `title` = ?, `code` = ?, `value` = ?, `symbol_left` = ?, `symbol_right` = ?, `decimal_place` = ? WHERE `currency_id` = ? ");
    	$statement->execute(array($data['title'], $data['code'], $data['value'], $data['symbol_left'], $data['symbol_right'], $data['decimal_place'], $currency_id));
	...
	public function getCurrency($currency_id, $store_id = null) 
	{
	    ...
	    $statement = $this->db->prepare("SELECT `currency`.*, `c2s`.`status`, `c2s`.`sort_order` 
	    	...
	    	WHERE `c2s`.`store_id` = ? AND (`currency`.`currency_id` = ? OR `currency`.`code` = ?)");
	    $statement->execute(array($store_id, $currency_id, $currency_id));
