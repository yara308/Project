...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'RECACHE')
{
  try {
    $code = trim($request->post['lang']);
    $statement->prepare("SELECT `id` FROM `languages` WHERE `code` = ?");
    $statement->execute(array($code));
    $lang = $statement->fetch(PDO::FETCH_ASSOC);
    if (!isset($lang['id'])) {
      throw new Exception(trans('error_language_not_found'));
    }
    $statement->prepare("SELECT `lang_key` as `key`, `lang_value` as `value` FROM `language_translations` WHERE lang_id = ?");
    $statement->execute(array($lang['id']));
    $trans = $statement->fetchAll(PDO::FETCH_ASSOC);
    header('Content-Type: application/json');
    echo json_encode(array('msg' => trans('text_trans_data'), 'trans' => $trans));
    exit();
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->post['action_type']) AND $request->post['action_type'] == 'ADDKEY')
{
  $key = trim($request->post['key']);
  $statement->prepare("SELECT `id` FROM `languages`");
  $statement->execute(array());
  $langs = $statement->fetchAll(PDO::FETCH_ASSOC);
  foreach ($langs as $lang) {
    $statement->prepare("SELECT `id` FROM `language_translations` WHERE `lang_id` = ? AND `lang_key` = ?");
    $statement->execute(array($lang['id'], $key));
    $lang_key = $statement->fetch(PDO::FETCH_ASSOC);
    if (!isset($lang_key['id'])) {
      $statement = db()->prepare("INSERT INTO `language_translations` SET `lang_id` = ?, `lang_key` = ?");
      $statement->execute(array($lang['id'], $key));
    }
  }
}
...
if ($request->server['REQUEST_METHOD'] == 'POST' AND isset($request->get['action_type']) AND $request->get['action_type'] == 'TRANSLATE')
{
  try {
    ...
    if (!$lang_value) {
      ...
      if (!$lang_value) {
        throw new Exception('error_language_value_in_english');
      }
