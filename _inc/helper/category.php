...
function getSubcategoriesByCategoryId($parentCategoryId, $store = null) {
    // Prepare the data array with the parent category filter.
    $data = array(
        'filter_parent_id' => $parentCategoryId
    );

    // Call the existing get_categorys function with the data and store parameters.
    return get_categorys($data, $store);
}
