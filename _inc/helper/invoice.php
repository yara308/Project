...
function get_postemplate_data($invoice_id)
{
    ...
    $data = array(
      ...
      'subtotal' => currency_format($invoice_info['subtotal'],$invoice_info['currency']),
      'discount_type' => ucfirst($invoice_info['discount_type']),
      'discount_amount' => currency_format($invoice_info['discount_amount'],$invoice_info['currency']),
      'interest_amount' => currency_format($invoice_info['interest_amount'],$invoice_info['currency']),
      'interest_percentage' => currency_format($invoice_info['interest_percentage'],$invoice_info['currency']),
      'item_tax' => currency_format($invoice_info['item_tax'],$invoice_info['currency']),
      'order_tax' => currency_format($invoice_info['order_tax'],$invoice_info['currency']),
      'total_tax' => currency_format($invoice_info['item_tax']+$invoice_info['order_tax'],$invoice_info['currency']),
      'cgst' => currency_format($invoice_info['cgst'],$invoice_info['currency']),
      'sgst' => currency_format($invoice_info['sgst'],$invoice_info['currency']),
      'igst' => currency_format($invoice_info['igst'],$invoice_info['currency']),
      'total_purchase_price' => currency_format($invoice_info['total_purchase_price'],$invoice_info['currency']),
      'shipping_type' => '',
      'shipping_amount' => currency_format($invoice_info['shipping_amount'],$invoice_info['currency']),
      'others_charge' => currency_format($invoice_info['others_charge'],$invoice_info['currency']),
      'payable_amount' => currency_format($invoice_info['payable_amount']+$invoice_info['previous_due'],$invoice_info['currency']),
      'paid_amount' => currency_format($invoice_info['paid_amount']+$invoice_info['prev_due_paid']+($invoice_info['return_amount']),$invoice_info['currency']),
      'due' => currency_format(($invoice_info['due']+$invoice_info['previous_due'])-$invoice_info['prev_due_paid'],$invoice_info['currency']),
      'due_paid' => currency_format($invoice_info['due_paid'],$invoice_info['currency']),
      'previous_due' => currency_format($invoice_info['previous_due'],$invoice_info['currency']),
      'prev_due_paid' => currency_format($invoice_info['prev_due_paid'],$invoice_info['currency']),
      'profit' => currency_format($invoice_info['profit'],$invoice_info['currency']),
      'return_amount' => currency_format($invoice_info['return_amount'],$invoice_info['currency']),
      'change' => currency_format($invoice_info['balance'],$invoice_info['currency']),
      ...
      foreach ($item as $key => $val) {
        if (in_array($key, array('sl'))) {
          $new_item[$key] = $val;
            ...
            if (in_array($key, array('item_quantity'))) {
                $new_item[$key] = currency_format($val);
            } else {
                $new_item[$key] = currency_format($val,$invoice_info['currency']);
            }
        }
      }
      $new_item['hsn_code'] = get_the_product($item['item_id'],'hsn_code');

      $product_info = get_the_product($item['item_id']);
      $new_item['hsn_code'] = $product_info['hsn_code'];
      
        // Image
        if (isset($product_info['p_image']) && ((FILEMANAGERPATH && is_file(FILEMANAGERPATH.$product_info['p_image']) && file_exists(FILEMANAGERPATH.$product_info['p_image'])) || (is_file(DIR_STORAGE . 'products' . $product_info['p_image']) && file_exists(DIR_STORAGE . 'products' . $product_info['p_image'])))) {
            $root_url = FILEMANAGERURL ? FILEMANAGERURL : root_url();
            $product_image = '<img  src="'.$root_url.'/'.$product_info['p_image'].'" width="30" height="30">';
        } else {
        
            $product_image = '<img src="../assets/itsolution24/img/noimage.jpg" width="30" height="30">';
        }
        $new_item['image'] = $product_image;
      ...
      foreach ($payment as $key => $val) {
        if (in_array($key, array('sl'))) {
          $new_payment[$key] = $val;
            ...
            $new_payment[$key] = $payment['total_paid'] > 0 ? currency_format($payment['total_paid'],$invoice_info['currency']) : currency_format($payment['amount'],$invoice_info['currency']);
        } elseif ($key == 'name') {
          ...
          $new_payment[$key] = currency_format($val,$invoice_info['currency']);
        }
      }
    ...
    foreach ($taxes as $tax) {
      ...
      foreach ($tax as $key => $val) {
        if (in_array($key, array('sl'))) {
          ...
          $new_tax[$key] = currency_format($val,$invoice_info['currency']);
        }
      }
      $tax_array[] = $new_tax;
      $inc++;
...
function generate_order_reference_no($reference_no = null)
{ 
    $store_id = store_id();
    $prfix = 'KUW';
    ...
    if (!$reference_no) {
        ...
        $store_id = $store_id ? $store_id : store_id();
        $statemtnt = registry()->get('db')->prepare("SELECT * FROM `quotation_info` WHERE `store_id` = ?  ORDER BY `info_id` DESC");
        $statemtnt->execute(array($store_id));
        $last_transaction = $statemtnt->fetch(PDO::FETCH_ASSOC);

        $reference_no = isset($last_transaction['reference_no']) ? $last_transaction['reference_no'] : $prfix . rand(1000, 9999);
    }
    $statement = registry()->get('db')->prepare("SELECT * FROM `quotation_info` WHERE `store_id` = ? AND `reference_no` = ?");
    $statement->execute(array($store_id, $reference_no));
    $order = $statement->fetch(PDO::FETCH_ASSOC);

    if (isset($order['reference_no'])) {
        $reference_no = $prfix . rand(1000, 9999);
        generate_order_reference_no($reference_no);
    } else {
        $reference_no = $prfix . rand(1000, 9999);
    }
    return $reference_no;
    ...
    $data = array(
      ...
      'items' => array(
        array(
           ...
           'unitName' => '',
           'sl' => 1,
           'image' => '<img src="../assets/itsolution24/img/noimage.jpg" width="30" height="30">',
        ),
      ),
      'return_items' => array(
        array(
