...
{    
    ...
    if (empty($request->post['reference_no'])) {
      throw new Exception(trans('error_reference_number_is_not_valid'));
    }

    // Validate Reference No
    if (!validateAlphanumeric($request->post['reference_no'])) {
      throw new Exception(trans('error_invalid_reference_number_allowed_only_alphanumeric'));
    }
    ...
    // Validate order tax
    if (!is_numeric($request->post['order-tax-amount'])) {
    ...
    // Validate tax amount
    if (!is_numeric($request->post['total-item-tax'])) {
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'CREATE')
{
  try {
    ...
    $action_cat = isset($request->get['action_cat']) ? $request->get['action_cat'] : '';
    $sup_id = $request->post['sup_id'];
    $supplier_info = get_the_supplier($sup_id);
    $created_at = date_time();

    $invoice_id = isset($request->post['invoice_id']) && $request->post['invoice_id'] ? $request->post['invoice_id'] : '';
    $invoice = get_the_purchase_invoice($invoice_id);
    if ($invoice) {
      $due = $invoice['due'];
      $paid_amount = $invoice['paid_amount'];

      $prev_items = array();
      $invoice_items = get_purchase_invoice_items($invoice_id);
      foreach ($invoice_items as $item) {
        ...
        $statement->execute(array($item['item_id'], $store_id));

        if ($item['item_variant_slug']) {
          $prev_items[$item['item_id']][$item['item_variant_slug']]['total_sell'] = $item['total_sell'];
          $prev_items[$item['item_id']][$item['item_variant_slug']]['return_quantity'] = $item['return_quantity'];
        } else {
          $prev_items[$item['item_id']]['total_sell'] = $item['total_sell'];
          $prev_items[$item['item_id']]['return_quantity'] = $item['return_quantity'];
        }
      }

      $statement = db()->prepare("DELETE FROM `purchase_info` WHERE `invoice_id` = ?");
      $statement->execute(array($invoice_id));

      $statement = db()->prepare("DELETE FROM `purchase_item` WHERE `invoice_id` = ?");
      $statement->execute(array($invoice_id));

      $statement = db()->prepare("DELETE FROM `purchase_price` WHERE `invoice_id` = ?");
      $statement->execute(array($invoice_id));


      $update_due = db()->prepare("UPDATE `supplier_to_store` SET `balance` = `balance`-{$due} WHERE `store_id` = ? AND `sup_id` = ?");
      $update_due->execute(array($store_id, $sup_id));

      $statement = db()->prepare("DELETE FROM `purchase_payments` WHERE `invoice_id` = ?");
      $statement->execute(array($invoice_id));

      $statement = db()->prepare("DELETE FROM `purchase_logs` WHERE `ref_invoice_id` = ?");
      $statement->execute(array($invoice_id));

      // Deposit
      if (($account_id = store('deposit_account_id')) && $paid_amount > 0) {
        $ref_no = unique_transaction_ref_no();
        $statement = db()->prepare("SELECT `source_id` FROM `income_sources` WHERE `for_purchase_edit_credit` = ?");
        $statement->execute(array(1));
        $source = $statement->fetch(PDO::FETCH_ASSOC);
        $source_id = $source['source_id'];
        $title = 'Deposit for purchase edit';
    ...
    $item_tax = $request->post['total-item-tax'];
    $order_tax = $request->post['order-tax-amount'] ? ($request->post['order-tax-amount'] / 100)*($subtotal-$item_tax) : 0;
    ...
    foreach ($request->post['products'] as  $product) {  
        ...
        $item_sell_price = $product['sell_price'];
        $item_selling_price = $product['sell_price'];
        $item_quantity = $product['quantity'];
        $item_ordered = $item_quantity;
        $item_tax = $product['tax_amount'];
        $tax_method = $product['tax_method'];
        $taxrate = $product['taxrate'];
        if ($tax_method == 'exclusive') {
            $item_total = ($item_quantity * (float)$item_purchase_price) + $item_tax;
        } else {
            $item_total = ($item_quantity * (float)$item_purchase_price);
        }
        ...
        if ($item_variant_slug) {
          if (isset($prev_items[$id][$item_variant_slug]['total_sell'])) {
            $total_sell = $prev_items[$id][$item_variant_slug]['total_sell'];
          } 
          if (isset($prev_items[$id][$item_variant_slug]['return_quantity'])) {
            $return_quantity = $prev_items[$id][$item_variant_slug]['return_quantity'];
          }          
        } else {
          if (isset($prev_items[$id]['total_sell'])) {
            $total_sell = $prev_items[$id]['total_sell'];
          }
          if (isset($prev_items[$id]['return_quantity'])) {
            $return_quantity = $prev_items[$id]['return_quantity'];
          }
        }
        
        // Insert purchase item
        $statement = db()->prepare("INSERT INTO `purchase_item` (invoice_id, store_id, sup_id, item_id, category_id, brand_id, item_name, item_variant_slug, item_variant_name, item_purchase_price, item_selling_price, item_ordered, item_quantity, total_sell, status, item_total, item_tax, tax_method, tax, gst, cgst, sgst, igst, return_quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $statement->execute(array($invoice_id, $store_id, $sup_id, $id, $category_id, $brand_id, $item_name, $item_variant_slug, $item_variant_name, $item_purchase_price, $item_selling_price, $item_ordered, $item_quantity, $total_sell, $status, $item_total, $item_tax, $tax_method, $taxrate, $taxrate, $cgst, $sgst, $igst, $return_quantity));
        
        // Update stock quantity
        $statement = db()->prepare("UPDATE `product_to_store` SET `quantity_in_stock` = `quantity_in_stock` + $item_quantity, `purchase_price` = ?, `sell_price` = ? WHERE `product_id` = ? AND `store_id` = ?");
        $statement->execute(array($item_purchase_price, $item_sell_price, $id, $store_id));
    }
...
if($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    $statement = db()->prepare("SELECT SUM(`due`) AS `due` FROM `purchase_price` WHERE `restaurant_id` = ? AND `invoice_id` = ?");
    $statement->execute(array($restaurant_id, $invoice_id));
    $invoice_price = $statement->fetch(PDO::FETCH_ASSOC);
    $due = $invoice_price['due'];
...
if($request->server['REQUEST_METHOD'] == 'POST' && $request->post['action_type'] == 'UPDATEINVOICEINFO')
{
    try {
        ...
        if (isset($request->post['items']) && !empty($request->post['items'])) {
          foreach ($request->post['items'] as $item_id => $item) {
            $receive_qty = $item['receive'];
            if ($receive_qty <= 0) {
              continue;
            }
            $the_item = $invoice_model->getTheInvoiceItem($invoice_id, $item_id);
            if (!$the_item) {
              throw new Exception(trans('error_item_not_found'));
            }
            if ($receive_qty > ($the_item['item_ordered']-$the_item['item_quantity'])) {
              throw new Exception(trans('error_receive_quantity_exceeds'));
            }
          }
        }

        // Insert receive quantity
        if (isset($request->post['items']) && !empty($request->post['items'])) {
          foreach ($request->post['items'] as $item_id => $item) {
            $receive_qty = $item['receive'];
            if ($receive_qty <= 0) {
            ...
            $the_item = $invoice_model->getTheInvoiceItem($invoice_id, $item_id);
            $statement = db()->prepare("UPDATE `purchase_item` SET `item_quantity` = `item_quantity` + $receive_qty WHERE `store_id` = ? AND `invoice_id` = ? AND `item_id` = ?");
            $statement->execute(array($store_id, $invoice_id, $item_id));

            $statement = db()->prepare("UPDATE `product_to_store` SET `quantity_in_stock` = `quantity_in_stock` + $receive_qty WHERE `store_id` = ? AND `product_id` = ?");
...
$columns = array(
    ...
    array(
        ...
        'formatter' => function($d, $row) {
            ...
            return '<a href="purchase.php?box_state=open&invoice_id='.$row['invoice_id'].'" class="btn btn-sm btn-block btn-info" title="'.trans('button_edit').'" data-loading-text="..."><span class="fa fa-pencil"></span></a>';     
        }
    ),
    array(
        'db' => 'invoice_id',
